export const keys = [
  [
    {'className': 'Backquote', 
      'eng': {'pressDown': '`', 'pressUp': '~', 'caps': '`', 'shiftCaps': '~'},
      'rus': {'pressDown': 'ё', 'pressUp': 'Ё', 'caps': 'Ё', 'shiftCaps': 'ё'}
    },
    {'className': 'Digit1', 
      'eng': {'pressDown': '1', 'pressUp': '!', 'caps': '1', 'shiftCaps': '!'},
      'rus': {'pressDown': '1', 'pressUp': '!', 'caps': '1', 'shiftCaps': '!'}
    },
    {'className': 'Digit2', 
      'eng': {'pressDown': '2', 'pressUp': '@', 'caps': '2', 'shiftCaps': '@'},
      'rus': {'pressDown': '2', 'pressUp': '"', 'caps': '2', 'shiftCaps': '"'}
    },
    {'className': 'Digit3',
      'eng': {'pressDown': '3', 'pressUp': '#', 'caps': '3', 'shiftCaps': '#'},
      'rus': {'pressDown': '3', 'pressUp': '№', 'caps': '3', 'shiftCaps': '№'}
    },
    {'className': 'Digit4',
      'eng': {'pressDown': '4', 'pressUp': '$', 'caps': '4', 'shiftCaps': '$'},
      'rus': {'pressDown': '4', 'pressUp': ';', 'caps': '4', 'shiftCaps': ';'}
    },
    {'className': 'Digit5',
      'eng': {'pressDown': '5', 'pressUp': '%', 'caps': '5', 'shiftCaps': '%'},
      'rus': {'pressDown': '5', 'pressUp': '%', 'caps': '5', 'shiftCaps': '%'}
    },
    {'className': 'Digit6',
      'eng': {'pressDown': '6', 'pressUp': '^', 'caps': '6', 'shiftCaps': '^'},
      'rus': {'pressDown': '6', 'pressUp': ':', 'caps': '6', 'shiftCaps': ':'}
    },
    {'className': 'Digit7',
      'eng': {'pressDown': '7', 'pressUp': '&', 'caps': '7', 'shiftCaps': '&'},
      'rus': {'pressDown': '7', 'pressUp': '?', 'caps': '7', 'shiftCaps': '?'}
    },
    {'className': 'Digit8',
      'eng': {'pressDown': '8', 'pressUp': '*', 'caps': '8', 'shiftCaps': '*'},
      'rus': {'pressDown': '8', 'pressUp': '*', 'caps': '8', 'shiftCaps': '*'}
    },
    {'className': 'Digit9', 
      'eng': {'pressDown': '9', 'pressUp': '(', 'caps': '9', 'shiftCaps': '('},
      'rus': {'pressDown': '9', 'pressUp': '(', 'caps': '9', 'shiftCaps': '('}
    },
    {'className': 'Digit0',
      'eng': {'pressDown': '0', 'pressUp': ')', 'caps': '0', 'shiftCaps': ')'},
      'rus': {'pressDown': '0', 'pressUp': ')', 'caps': '0', 'shiftCaps': ')'}
    },
    {'className': 'Minus',
      'eng': {'pressDown': '-', 'pressUp': '_', 'caps': '-', 'shiftCaps': '_'},
      'rus': {'pressDown': '-', 'pressUp': '_', 'caps': '-', 'shiftCaps': '_'}
    },
    {'className': 'Equal',
      'eng': {'pressDown': "=", 'pressUp': '+', 'caps': "=", 'shiftCaps': '+'},
      'rus': {'pressDown': '=', 'pressUp': '+', 'caps': '=', 'shiftCaps': '+'}
    },
    {'className': 'Backspace',
      'eng': {'pressDown': 'Backspace', 'pressUp': 'Backspace'},
      'rus': {'pressDown': 'Backspace', 'pressUp': 'Backspace'}
    },
  ],

  [
    {'className': 'Tab', 
      'eng': {'pressDown': 'Tab', 'pressUp': 'Tab'},
      'rus': {'pressDown': 'Tab', 'pressUp': 'Tab'}
    },
    {'className': 'KeyQ', 
      'eng': {'pressDown': 'q', 'pressUp': 'Q', 'caps': "Q", 'shiftCaps': 'q'},
      'rus': {'pressDown': 'й', 'pressUp': 'Й'}
    },
    {'className': 'KeyW', 
      'eng': {'pressDown': 'w', 'pressUp': 'W'},
      'rus': {'pressDown': 'ц', 'pressUp': 'Ц'}
    },
    {'className': 'KeyE',
      'eng': {'pressDown': 'e', 'pressUp': 'E'},
      'rus': {'pressDown': 'у', 'pressUp': 'У'}
    },
    {'className': 'KeyR',
      'eng': {'pressDown': 'r', 'pressUp': 'R'},
      'rus': {'pressDown': 'к', 'pressUp': 'К'}
    },
    {'className': 'KeyT',
      'eng': {'pressDown': 't', 'pressUp': 'T'},
      'rus': {'pressDown': 'е', 'pressUp': 'Е'}
    },
    {'className': 'KeyY',
      'eng': {'pressDown': 'y', 'pressUp': 'Y'},
      'rus': {'pressDown': 'н', 'pressUp': 'Н'}
    },
    {'className': 'KeyU',
      'eng': {'pressDown': 'u', 'pressUp': 'U'},
      'rus': {'pressDown': 'г', 'pressUp': 'Г'}
    },
    {'className': 'KeyI',
      'eng': {'pressDown': 'i', 'pressUp': 'I'},
      'rus': {'pressDown': 'ш', 'pressUp': 'Ш'}
    },
    {'className': 'KeyO', 
      'eng': {'pressDown': 'o', 'pressUp': 'O'},
      'rus': {'pressDown': 'щ', 'pressUp': 'Щ'}
    },
    {'className': 'KeyP',
      'eng': {'pressDown': 'p', 'pressUp': 'P'},
      'rus': {'pressDown': 'з', 'pressUp': 'З'}
    },
    {'className': 'BracketLeft',
      'eng': {'pressDown': '[', 'pressUp': '{', 'caps': '[', 'shiftCaps': '{'},
      'rus': {'pressDown': 'х', 'pressUp': 'Х', 'caps': 'Х', 'shiftCaps': 'х'}
    },
    {'className': 'BracketRight',
      'eng': {'pressDown': ']', 'pressUp': '}', 'caps': ']', 'shiftCaps': '}'},
      'rus': {'pressDown': 'ъ', 'pressUp': 'Ъ', 'caps': 'Ъ', 'shiftCaps': 'ъ'}
    },
    {'className': 'Backslash',
      'eng': {'pressDown': '\\', 'pressUp': '|', 'caps': '\\', 'shiftCaps': '|'},
      'rus': {'pressDown': '\\', 'pressUp': '/', 'caps': '\\', 'shiftCaps': '/'}
    },
    {'className': 'Delete',
      'eng': {'pressDown': 'Del', 'pressUp': 'Del'},
      'rus': {'pressDown': 'Del', 'pressUp': 'Del'}
    },
  ],

  [
    {'className': 'CapsLock', 
      'eng': {'pressDown': 'CapsLock', 'pressUp': 'CapsLock'},
      'rus': {'pressDown': 'CapsLock', 'pressUp': 'CapsLock'}
    },
    {'className': 'KeyA', 
      'eng': {'pressDown': 'a', 'pressUp': 'A'},
      'rus': {'pressDown': 'й', 'pressUp': 'Й'}
    },
    {'className': 'KeyS', 
      'eng': {'pressDown': 's', 'pressUp': 'S'},
      'rus': {'pressDown': 'ц', 'pressUp': 'Ц'}
    },
    {'className': 'KeyE',
      'eng': {'pressDown': 'e', 'pressUp': 'E'},
      'rus': {'pressDown': 'у', 'pressUp': 'У'}
    },
    {'className': 'KeyD',
      'eng': {'pressDown': 'd', 'pressUp': 'D'},
      'rus': {'pressDown': 'к', 'pressUp': 'К'}
    },
    {'className': 'KeyF',
      'eng': {'pressDown': 'f', 'pressUp': 'F'},
      'rus': {'pressDown': 'е', 'pressUp': 'Е'}
    },
    {'className': 'KeyG',
      'eng': {'pressDown': 'g', 'pressUp': 'G'},
      'rus': {'pressDown': 'н', 'pressUp': 'Н'}
    },
    {'className': 'KeyH',
      'eng': {'pressDown': 'h', 'pressUp': 'H'},
      'rus': {'pressDown': 'г', 'pressUp': 'Г'}
    },
    {'className': 'KeyJ',
      'eng': {'pressDown': 'j', 'pressUp': 'J'},
      'rus': {'pressDown': 'ш', 'pressUp': 'Ш'}
    },
    {'className': 'KeyK', 
      'eng': {'pressDown': 'k', 'pressUp': 'K'},
      'rus': {'pressDown': 'щ', 'pressUp': 'Щ'}
    },
    {'className': 'KeyL',
      'eng': {'pressDown': 'l', 'pressUp': 'L'},
      'rus': {'pressDown': 'з', 'pressUp': 'З'}
    },
    {'className': 'Semicolon',
      'eng': {'pressDown': ';', 'pressUp': ':', 'caps': ';', 'shiftCaps': ':'},
      'rus': {'pressDown': 'ж', 'pressUp': 'Ж', 'caps': 'Ж', 'shiftCaps': 'ж'}
    },
    {'className': 'Quote',
      'eng': {'pressDown': "'", 'pressUp': '"', 'caps': "'", 'shiftCaps': '"'},
      'rus': {'pressDown': 'э', 'pressUp': 'Э', 'caps': 'Э', 'shiftCaps': 'э'}
    },
    {'className': 'Enter',
      'eng': {'pressDown': 'Enter', 'pressUp': 'Enter'},
      'rus': {'pressDown': 'Enter', 'pressUp': 'Enter'}
    },
  ],

  [
    {'className': 'ShiftLeft', 
      'eng': {'pressDown': 'Shift', 'pressUp': 'Shift'},
      'rus': {'pressDown': 'Shift', 'pressUp': 'Shift'}
    },
    {'className': 'KeyZ', 
      'eng': {'pressDown': 'z', 'pressUp': 'Z'},
      'rus': {'pressDown': 'я', 'pressUp': 'Я'}
    },
    {'className': 'KeyX', 
      'eng': {'pressDown': 'x', 'pressUp': 'X'},
      'rus': {'pressDown': 'ч', 'pressUp': 'Ч'}
    },
    {'className': 'KeyC',
      'eng': {'pressDown': 'c', 'pressUp': 'C'},
      'rus': {'pressDown': 'с', 'pressUp': 'С'}
    },
    {'className': 'KeyV',
      'eng': {'pressDown': 'v', 'pressUp': 'V'},
      'rus': {'pressDown': 'м', 'pressUp': 'М'}
    },
    {'className': 'KeyB',
      'eng': {'pressDown': 'b', 'pressUp': 'B'},
      'rus': {'pressDown': 'и', 'pressUp': 'И'}
    },
    {'className': 'KeyN',
      'eng': {'pressDown': 'n', 'pressUp': 'N'},
      'rus': {'pressDown': 'т', 'pressUp': 'Т'}
    },
    {'className': 'KeyM',
      'eng': {'pressDown': 'm', 'pressUp': 'M'},
      'rus': {'pressDown': 'ь', 'pressUp': 'Ь'}
    },
    {'className': 'Comma',
      'eng': {'pressDown': ',', 'pressUp': '<', 'caps': ',', 'shiftCaps': '<'},
      'rus': {'pressDown': 'б', 'pressUp': 'Б', 'caps': 'Б', 'shiftCaps': 'б'},
    },
    {'className': 'Period', 
      'eng': {'pressDown': '.', 'pressUp': '>', 'caps': '.', 'shiftCaps': '>'},
      'rus': {'pressDown': 'ю', 'pressUp': 'Ю', 'caps': 'Ю', 'shiftCaps': 'ю'}
    },
    {'className': 'Slash',
      'eng': {'pressDown': '/', 'pressUp': '?', 'caps': '/', 'shiftCaps': '?'},
      'rus': {'pressDown': '.', 'pressUp': ',', 'caps': '.', 'shiftCaps': ','}
    },
    {'className': 'ArrowUp',
      'eng': {'pressDown': '▲', 'pressUp': '▲'},
      'rus': {'pressDown': '▲', 'pressUp': '▲'}
    },
    {'className': 'ShiftRight', 
      'eng': {'pressDown': 'Shift', 'pressUp': 'Shift'},
      'rus': {'pressDown': 'Shift', 'pressUp': 'Shift'}
    },
  ],

  [
    {'className': 'ControlLeft', 
      'eng': {'pressDown': 'Ctrl', 'pressUp': 'Ctrl'},
      'rus': {'pressDown': 'Ctrl', 'pressUp': 'Ctrl'}
    },
    {'className': 'MetaLeft', 
      'eng': {'pressDown': 'Win', 'pressUp': 'Win'},
      'rus': {'pressDown': 'Win', 'pressUp': 'Win'}
    },
    {'className': 'AltLeft', 
      'eng': {'pressDown': 'Alt', 'pressUp': 'Alt'},
      'rus': {'pressDown': 'Alt', 'pressUp': 'Alt'}
    },
    {'className': 'Space',
      'eng': {'pressDown': ' ', 'pressUp': ' '},
      'rus': {'pressDown': ' ', 'pressUp': ' '}
    },
    {'className': 'AltRight',
      'eng': {'pressDown': 'Alt', 'pressUp': 'Alt'},
      'rus': {'pressDown': 'Alt', 'pressUp': 'Alt'}
    },
    {'className': 'ArrowLeft',
      'eng': {'pressDown': '◀', 'pressUp': '◀'},
      'rus': {'pressDown': '◀', 'pressUp': '◀'}
    },
    {'className': 'ArrowDown',
      'eng': {'pressDown': '▼', 'pressUp': '▼'},
      'rus': {'pressDown': '▼', 'pressUp': '▼'}
    },
    {'className': 'ArrowRight',
      'eng': {'pressDown': '▶', 'pressUp': '▶'},
      'rus': {'pressDown': '▶', 'pressUp': '▶'}
    },
    {'className': 'ControlRight',
      'eng': {'pressDown': 'Ctrl', 'pressUp': 'Ctrl'},
      'rus': {'pressDown': 'Ctrl', 'pressUp': 'Ctrl'},
    },
  ],
]